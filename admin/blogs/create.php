  ...
  <form method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow p-8 space-y-5">
    ...
    <div>
      ...
      <textarea name="content" id="content" rows="12"
        class="w-full mt-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none font-mono text-sm"><?= htmlspecialchars($_POST['content'] ?? '', ENT_QUOTES) ?></textarea>
      <p class="text-xs text-gray-400 mt-1">HTML is supported.</p>
    </div>
    <div class="flex items-center gap-2">
      <input type="checkbox" name="published" id="published" value="1" <?= isset($_POST['published']) ? 'checked' : '' ?> class="w-4 h-4">
      <label for="published" class="text-sm font-medium text-gray-700">Publish immediately</label>
    </div>
    <div class="flex gap-3">
      <button class="bg-blue-700 text-white px-6 py-2 rounded-lg hover:bg-blue-600 font-semibold">Create Blog</button>
