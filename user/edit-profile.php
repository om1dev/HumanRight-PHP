...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'profile') {
        ...
        if (!$username) {
            ...
            $existing = $db->users->findOne(['username' => $username, '_id' => ['$ne' => new MongoDB\BSON\ObjectId($userId)]]);
            if ($existing) {
                ...
                if (!empty($_FILES['photo']['name']) && $_FILES['photo']['error'] === 0) {
                    ...
                    if (!in_array($ext, $allowed)) {
                        ...
                        try {
                            $photoUrl = uploadToCloudinary($_FILES['photo']['tmp_name'], 'humanrights/users');
...
<div class="max-w-3xl mx-auto px-4 py-12">
  <div class="flex items-center gap-3 mb-8">
    <a href="<?= SITE_URL ?>/user/profile" class="text-blue-600 hover:underline text-sm">
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="occupation" value="<?= sanitize($user['occupation'] ?? '') ?>"
            placeholder="e.g. Social Worker, Lawyer"
            class="w-full mt-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
        <div>
          <label class="text-sm font-medium text-gray-700">Location</label>
          <input type="text" name="location" value="<?= sanitize($user['location'] ?? '') ?>"
            placeholder="City, Country"
            class="w-full mt-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
        </div>
        <div>
          <label class="text-sm font-medium text-gray-700">Website</label>
          <input type="url" name="website" value="<?= sanitize($user['website'] ?? '') ?>"
      ...
      <div>
        ...
        <textarea name="bio" rows="3" placeholder="Tell us about yourself..."
          class="w-full mt-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm resize-none"><?= sanitize($user['bio'] ?? '') ?></textarea>
      </div>
      ...
      <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label class="text-xs text-gray-500 flex items-center gap-1 mb-1"><i class="fa-brands fa-twitter text-sky-500"></i> Twitter</label>
            <input type="url" name="twitter" value="<?= sanitize($user['twitter'] ?? '') ?>"
              placeholder="https://twitter.com/..."
              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
          </div>
          <div>
            <label class="text-xs text-gray-500 flex items-center gap-1 mb-1"><i class="fa-brands fa-instagram text-pink-500"></i> Instagram</label>
            <input type="url" name="instagram" value="<?= sanitize($user['instagram'] ?? '') ?>"
